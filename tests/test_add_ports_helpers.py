...
class TestShouldSkipMarker:
    def test_skip_below_min_area(self) -> None:
        assert _should_skip_marker(
            1.0,
            1.0,
            min_pin_area_um2=2.0,
            max_pin_area_um2=None,
            skip_square_ports=False,
        )
    ...
    def test_skip_above_max_area(self) -> None:
        assert _should_skip_marker(
            10.0,
            10.0,
            min_pin_area_um2=None,
            max_pin_area_um2=50.0,
            skip_square_ports=False,
        )
    ...
    def test_skip_square_ports(self) -> None:
        assert _should_skip_marker(
            ...
            min_pin_area_um2=None,
            max_pin_area_um2=None,
            skip_square_ports=True,
        )

    def test_no_skip_rectangular(self) -> None:
        assert not _should_skip_marker(
            1.0,
            2.0,
            min_pin_area_um2=None,
            max_pin_area_um2=None,
    ...
    def test_no_skip_within_bounds(self) -> None:
        assert not _should_skip_marker(
            2.0,
            3.0,
            min_pin_area_um2=1.0,
            max_pin_area_um2=100.0,
            skip_square_ports=False,
        )
...
class TestInferPortDirection:
    ...
    BOUNDS: ClassVar[dict[str, Any]] = dict(
        xc=50.0, yc=50.0, dxmin=0.0, dymin=0.0, dxmax=100.0, dymax=100.0, tol=0.1
    )

    def test_east_rectangular(self) -> None:
        """Tall marker on the right side -> east (0 degrees)."""
        orient, width, _x, _y = _infer_port_direction(
            x=80.0,
            y=50.0,
            dx=2.0,
            dy=10.0,
    ...
    def test_square_east_boundary(self) -> None:
        ...
        orient, _width, _, _ = _infer_port_direction(
            x=99.95,
            y=50.0,
            dx=5.0,
            dy=5.0,
...
class TestApplyInsidePosition:
    BBOX: ClassVar[dict[str, Any]] = dict(
        pxmin=10.0, pymin=20.0, pxmax=30.0, pymax=40.0
    )
    ...
    def test_east_inside(self) -> None:
        x, _y = _apply_inside_position(0.0, 20.0, 30.0, **self.BBOX, inside=True)
        assert x == 30.0  # pxmax

    def test_west_inside(self) -> None:
        x, _y = _apply_inside_position(180.0, 20.0, 30.0, **self.BBOX, inside=True)
        assert x == 10.0  # pxmin

    def test_north_inside(self) -> None:
        _x, y = _apply_inside_position(90.0, 20.0, 30.0, **self.BBOX, inside=True)
        assert y == 40.0  # pymax

    def test_south_inside(self) -> None:
        _x, y = _apply_inside_position(270.0, 20.0, 30.0, **self.BBOX, inside=True)
        assert y == 20.0  # pymin

    def test_east_inside_opposite(self) -> None:
        x, _y = _apply_inside_position(
            0.0, 20.0, 30.0, **self.BBOX, inside=True, use_opposite_side=True
        )
        assert x == 10.0  # pxmin (opposite)

    def test_west_inside_opposite(self) -> None:
        x, _y = _apply_inside_position(
            180.0, 20.0, 30.0, **self.BBOX, inside=True, use_opposite_side=True
        )
        assert x == 30.0  # pxmax (opposite)

    def test_north_inside_opposite(self) -> None:
        _x, y = _apply_inside_position(
            90.0, 20.0, 30.0, **self.BBOX, inside=True, use_opposite_side=True
        )
        assert y == 20.0  # pymin (opposite)

    def test_south_inside_opposite(self) -> None:
        _x, y = _apply_inside_position(
            270.0, 20.0, 30.0, **self.BBOX, inside=True, use_opposite_side=True
        )
