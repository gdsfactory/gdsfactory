...
from gdsfactory.components.vias.via_stack import via_stack_deprecated


def test_default_call_settings_snapshot() -> None:
    c = via_stack_deprecated()
...
def test_legacy_positional_call_still_works_with_warning() -> None:
    with warnings.catch_warnings(record=True) as caught:
        ...
        c = via_stack_deprecated((5.0, 5.0), ("M1", "M2"), None, ("via1", None))
...
def test_bottom_top_layer_convenience() -> None:
    c = via_stack_deprecated(bottom_layer="M1", top_layer="M2", via_between="via1")
    d = c.to_dict(with_ports=True)
...
def test_bottom_top_layer_rejects_mixing_with_layers() -> None:
    ...
    with pytest.raises(ValueError):
        via_stack_deprecated(bottom_layer="M1", top_layer="M2", layers=("M1", "M2"))


def test_columns_rows_cap_via_count() -> None:
    c_uncapped = via_stack_deprecated(size=(20.0, 20.0))
    c_capped = via_stack_deprecated(size=(20.0, 20.0), columns=1, rows=1)
