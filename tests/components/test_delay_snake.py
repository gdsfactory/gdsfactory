...
def test_length_delay_snake() -> None:
    length = 200.0
    c = gf.c.delay_snake(n=2, length=length, length0=50, cross_section="xs_sc")
...
def test_length_delay_snake2() -> None:
    length = 200.0
    c = gf.c.delay_snake2(length=length, cross_section="xs_sc")
...
def test_delay_snake_sbend_length() -> None:
    length = 200.0
    c = gf.c.delay_snake_sbend(length=length, cross_section="xs_sc")
    length_computed = c.area(layer=(1, 0)) / 0.5
    np.isclose(length, length_computed)
