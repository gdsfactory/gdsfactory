...
class TestVirtualInstances:
    """Test virtual instance functionality in YAML."""

    def test_virtual_instance_creation(self) -> None:
        """Test that virtual instances are created correctly."""
        yaml_content = """
        name: test_virtual
        ...
        ports:
          o1: real_mmi,o1
          o2: real_mmi,o2
          virtual_o1: virtual_mmi,o1
          virtual_o2: virtual_mmi,o2
        """

        import gdsfactory as gf

        c = gf.read.from_yaml(yaml_content)

        # Check that we have both real and virtual instances
        assert len(c.insts) == 2

        # Check that one instance is virtual (has "virtual_" prefix in name)
        virtual_instances = [inst for inst in c.insts if "virtual_" in inst.cell.name]
        real_instances = [inst for inst in c.insts if "virtual_" not in inst.cell.name]

        assert len(virtual_instances) == 1
        assert len(real_instances) == 1

        # Virtual instance should still have ports available
        assert len(c.ports) == 4

        # Virtual instances should have minimal geometry compared to real ones
        virtual_inst = virtual_instances[0]
        real_inst = real_instances[0]

        # Both should have ports but virtual should have no actual geometry
        assert len(virtual_inst.ports) > 0
        assert len(real_inst.ports) > 0
