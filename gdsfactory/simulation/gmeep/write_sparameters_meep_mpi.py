...
def write_sparameters_meep_mpi(
    ...
    **kwargs,
    ...
    parameters_file = tempfile.with_suffix(".json")
    kwargs.update(filepath=str(filepath))

    layer_stack_json = layer_stack.json()
    parameters_file.write_text(layer_stack_json)
    ...
    script_lines = [
        ...
        f"\twith open('{parameters_file}') as jsonfile:\n",
        "\t\td = json.load(jsonfile)\n",
        f"\twrite_sparameters_meep(component = component, overwrite = {overwrite}, layer_stack = LayerStack(d))",
    ]
