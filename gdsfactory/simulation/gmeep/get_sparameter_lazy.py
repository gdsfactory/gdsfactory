...
def get_sparametersNxN(
    ...
    **settings,
    ...
    # if lazy_parallelism:
    #     n = mp.divide_parallel_processes(len(Sparams_indices), Sparams_dict=Sparams_dict)
    #     simulate(Sparams_indices[n])
    # else:
    #      for port_index in Sparams_indices:
    #         simulate(port_index, Sparams_dict=Sparams_dict)
...
if __name__ == "__main__":
    ...
    df = get_sparametersNxN(c, overwrite=True, animate=False, lazy_parallelism=True)
    # df.to_csv("df_lazy.csv", index=False)
    # plot_sparameters(df)
    # plt.show()
