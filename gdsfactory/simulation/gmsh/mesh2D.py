...
def get_xsection_bounds_inplane(
    ...
    for layer in component.get_layers():
        ...
        for polygon in polygons:
            ...
            min_point = (polygon[distances_sort_index[0]] + polygon[distances_sort_index[1]])/2
            max_point = (polygon[distances_sort_index[2]] + polygon[distances_sort_index[3]])/2
    ...
    for layer, inplane_bounds_list in inplane_bounds_dict.items():
        ...
        for inplane_bounds in inplane_bounds_list:
            if (
                ...
                zmax = zmin + height
                outplane_bounds_list.append({"umin": umin, "umax": umax, "zmin": zmin, "zmax": zmax})
...
def mesh2D(
    ...
    padding: Tuple[float, float, float, float] = (2., 2., 2., 2.),
    layer_stack: Optional[LayerStack] = None,
    exclude_layers: Optional[Tuple[Layer, ...]] = None,
):
    """Performs 2D meshing of component along cross-sectional line (x1,y1), (x2,y2).
    
    From http://jsdokken.com/src/pygmsh_tutorial.html
    ...
    # Background oxide
    points = [model.add_point([umin, zmin], mesh_size=base_resolution),
    ...
    for layer in component.get_layers():
        ...
        if (
            ...
            for bounds in bounds_dict[layer]:
                points = [[bounds["umin"], bounds["zmin"]], 
                            [bounds["umin"], bounds["zmax"]],
                            [bounds["umax"], bounds["zmax"]],
                            [bounds["umax"], bounds["zmin"]]]
                polygon = model.add_polygon(points, mesh_size=refine_resolution[layer] if refine_resolution else base_resolution)
                # for line in polygon.lines:
                #     model.in_surface(line, plane_surface)
...
if __name__ == "__main__":
    ...
    geometry = mesh2D(heaters, [(25, -2), (25, 25)], exclude_layers=[(1,10)], refine_resolution={(1,0): 0.02, (47,0): 0.07})
