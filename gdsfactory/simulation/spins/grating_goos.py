...
out_folder_name = "grating_full_opt"
...
with plan:
    ...
    waveguide = goos.Cuboid(
        ...
        extents=goos.Constant([params.wg_len, params.wg_width, params.wg_thickness]),
        material=goos.material.Material(index=params.eps_wg),
    )
...
with plan:
    ...
    sim_z_extent = (
        params.wg_thickness + params.beam_dist + params.box_size + 2000 + pml_thick * 2
    )
...
with open(os.path.join(folder_plt, "step1.pkl"), "rb") as fp:
    ...
    plt.figure(figsize=(10, 12))
    plt.imshow(
        np.rot90(np.abs(data["monitor_data"]["sim_cont.field"][1].squeeze()), 1, (0, 1))
    )
    plt.axis("off")
    plt.tight_layout()
    plt.show()
