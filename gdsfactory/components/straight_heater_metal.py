...
def straight_heater_metal_undercut(
    ...
    **kwargs,
    ...
    if via_stack:
        refs = list(sequence.named_references.get_inst_names().keys())
        via_stackw = gf.get_component(via_stack)
        sequence.named_references[refs[0]].size_info.cw
        sequence.named_references[refs[-1]].size_info.ce
        via_stackw.get_ports_xsize() / 2 + heater_taper_length or 0
