...
def _bend_modified_hermite(
    ...
    """
    if radius <= 0:
        raise ValueError(f"Error: radius must be positive, but it is {radius}.")
    if not (0 < angle < 360):
        raise ValueError(
            f"Error: angle must be between 0 and 360 exclusive, but angle is {angle}."
        )
    if inner_tangent_magnitude <= 0:
        raise ValueError(
            f"Error: inner_tangent_magnitude must be positive, but it is {inner_tangent_magnitude}."
        )
    if outer_tangent_magnitude <= 0:
        raise ValueError(
            f"Error: outer_tangent_magnitude must be positive, but it is {outer_tangent_magnitude}."
        )
    ...
    inner_bend_points = _hermite_curve(
        t=t,
    ...
    outer_bend_points = _hermite_curve(
        t=t,
