...
def _compute_parameters(
    r_bend: float, wrap_angle_deg: float, radius: float
...
def disk(
    ...
    r_bend, size_x, dy, bus_length = _compute_parameters(r_bend, wrap_angle_deg, radius)

    c, bend_input, bend_middle, bend_output = _generate_bends(
        c, r_bend, wrap_angle_deg, xs
    )

    c, straight_left, straight_right = _generate_straights(
        c, bus_length, size_x, bend_input, bend_output, xs
    )
