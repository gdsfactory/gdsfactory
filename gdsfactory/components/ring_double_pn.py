...
def ring_double_pn(
    ...
    cross_section: CrossSection = partial(gf.cross_section.strip,sections=(Section(width=2*2.425, layer="SLAB90", name="slab"),),),
    pn_cross_section: CrossSection = gf.partial(gf.cross_section.pn, width_doping=2.425, width_slab=2*2.425, layer_via="VIAC",width_via=0.5, layer_metal="M1",width_metal=0.5, ),
    ...
    heater_vias: ComponentSpec = gf.partial(gf.components.via_stack,size=(0.5,0.5),layers=("M1","M2"),vias=(
        gf.partial(gf.components.via,layer='VIAC', size=(0.1,0.1), spacing=(0.2,0.2), enclosure=0.1),
        gf.partial(gf.components.via,layer='VIA1',size=(0.1,0.1), spacing=(0.2,0.2),enclosure=0.1)
    )),
    **kwargs,
) -> gf.Component:
    ...
    add_waveguide_path.append(gf.path.straight(length=2*radius*np.sin(np.pi/360*undoping_angle)))
    add_waveguide = c << add_waveguide_path.extrude(cross_section=cross_section)
    add_waveguide.x = 0
    add_waveguide.y = 0

    drop_waveguide_path = gf.Path()
    drop_waveguide_path.append(gf.path.straight(length=2*radius*np.sin(np.pi/360*undoping_angle)))
    ...
    if doped_heater:
        ...
        top_heater_ref.ymax = drop_waveguide.y - (doped_heater_waveguide_offset + doped_heater_width/2 + drop_gap)
        ...
        bottom_heater_ref = c << heater_path.extrude(width=0.5,layer=doped_heater_layer)
        bottom_heater_ref.rotate(-(undoping_angle-doped_heater_angle_buffer)/2)
        bottom_heater_ref.x = add_waveguide.x
        bottom_heater_ref.ymin = doped_heater_waveguide_offset + doped_heater_width/2 + add_gap

        bottom_left_heater_via = c << heater_vias()
        bottom_left_heater_via.rotate(bottom_heater_ref.ports['o1'].orientation)

        deltax = -abs(bottom_heater_ref.ports['o1'].x - bottom_left_heater_via.ports['e3'].x)
        deltay = abs(bottom_heater_ref.ports['o1'].y - bottom_left_heater_via.ports['e3'].y)
        ...
        deltax = abs(bottom_heater_ref.ports['o2'].x - bottom_right_heater_via.ports['e3'].x)
        deltay = abs(bottom_heater_ref.ports['o2'].y - bottom_right_heater_via.ports['e3'].y)
