...
def Cband_silicon_2x2MMI(
    add_sbend: bool = False,
    s_bend: ComponentSpec = bend_s,
    ) -> Component:
    'A C_band 2x2MMI in 220nm thick silicon copied from: https://opg.optica.org/oe/fulltext.cfm?uri=oe-25-23-28957&id=376719'
    def mmi_widths(t):
    ...
    top_input_block = c << gf.components.taper(length=1,width1=0.5,width2=0.7,layer="WG").move((-1,0.45))
    bottom_input_block = c << gf.components.taper(length=1,width1=0.5,width2=0.7,layer="WG").move((-1,-0.45))

    top_output_block = c << gf.components.taper(length=1,width1=0.7,width2=0.5,layer="WG").move((8,0.45))
    bottom_output_block = c << gf.components.taper(length=1,width1=0.7,width2=0.5,layer="WG").move((8,-0.45))

    if add_sbend == False:
        c.ports = {} #Remove ports from extruded elements
        ...
        top_input_sbend_ref = c << s_bend(); top_input_sbend_ref.mirror([0,1])
        bottom_input_sbend_ref = c << s_bend(); 
        top_output_sbend_ref = c << s_bend(); 
        bottom_output_sbend_ref = c << s_bend(); bottom_output_sbend_ref.mirror([0,1])
        ...
        bottom_output_sbend_ref.connect("o1",destination=bottom_output_block.ports['o2'])

        c.add_port("o1",port=top_input_sbend_ref.ports['o1'])
        c.add_port("o2",port=top_output_sbend_ref.ports['o2'])
        c.add_port("o3",port=bottom_output_sbend_ref.ports['o2'])
        c.add_port("o4",port=bottom_input_sbend_ref.ports['o1'])
    return c

if __name__ == "__main__":
    mmi = Cband_silicon_2x2MMI(add_sbend=True)
    mmi.show()