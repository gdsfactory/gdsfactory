...
def test_path():
    P = gdsfactory.Path()
    P.append(gdsfactory.path.arc(radius=10, angle=90))  # Circular arc
    P.append(gdsfactory.path.straight(length=10))  # Straight section
    P.append(
        gdsfactory.path.euler(radius=3, angle=-90)
    )  # Euler bend (aka "racetrack" curve)
    P.append(gdsfactory.path.straight(length=40))
    P.append(gdsfactory.path.arc(radius=8, angle=-45))
    P.append(gdsfactory.path.straight(length=10))
    P.append(gdsfactory.path.arc(radius=8, angle=45))
    P.append(gdsfactory.path.straight(length=10))
    P.length()
    P.length()
...
def rename():
    ...
    straight = gdsfactory.path.extrude(p, cross_section=X)
    return straight
...
def no_rename():
    ...
    # Combine the Path and the CrossSection
    straight = gdsfactory.path.extrude(p, cross_section=X, rename_ports=False)
    return straight
...
def double_loop():
    ...
    P.append(gdsfactory.path.straight())
    P.append(gdsfactory.path.arc(radius=5, angle=-90))
    ...
    c = gdsfactory.path.extrude(P, X, simplify=0.3)
    return c
...
def transition():
    ...
    Xtrans = gdsfactory.path.transition(
        cross_section1=X1, cross_section2=X2, width_type="sine"
    )

    P1 = gdsfactory.path.straight(length=5)
    P2 = gdsfactory.path.straight(length=5)
    wg1 = gdsfactory.path.extrude(P1, X1)
    wg2 = gdsfactory.path.extrude(P2, X2)

    P4 = gdsfactory.path.euler(radius=25, angle=45, p=0.5, use_eff=False)
    wg_trans = gdsfactory.path.extrude(P4, Xtrans)
...
component_factory = gdsfactory.types.get_name_to_function_dict(
...
def test_layers():
    P = gdsfactory.path.straight(length=10.001)
    X = gdsfactory.CrossSection()
    X.add(width=0.5, offset=0, layer=gdsfactory.LAYER.SLAB90, ports=["in", "out"])
    c = gdsfactory.path.extrude(P, X, simplify=5e-3, snap_to_grid_nm=5)
    assert c.ports["W0"].layer == gdsfactory.LAYER.SLAB90
