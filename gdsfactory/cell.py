...
def cell(
    ...
    *,
    ...
    default_decorator: Callable | None = None,
) -> Callable[[_F], _F]:
    ...
    def decorator_cell(func: _F) -> _F:
        ...
        def _cell(*args, **kwargs):
            ...
            passed_args_list = [
                f"{key}={clean_value_name(changed2[key])}"
                for key in sorted(changed.keys())
            ]

            if naming_style == "updk":
                full_args_list = [
                    f"{key}={clean_value_name(full[key])}"
                ...
                if changed_arg_list or include_module:
                    if include_module and changed_arg_list:
                        named_args_module_string = (
                            f"{named_args_string}_{func.__module__}"
                        )
                    elif include_module:
    ...
    return decorator_cell if func is None else decorator_cell(func)
