...
# Chebyshev-basis monomial coefficients for Fresnel integral evaluation.
# x(t) = t   * Horner(_EULER_F, t**4)
# y(t) = t**3 * Horner(_EULER_G, t**4)
# Domain: t in [0, 2.0], covering bends up to ~230 deg (p=1.0).
# Degree-6 Chebyshev achieves < 0.001 nm on a 10 um radius 90 deg bend.
# Coefficients generated by minimax fit against 50-digit mpmath reference.
_EULER_F: list[float] = [
    0.99999998960479,
    -0.09999993603363509,
...
def _fresnel_horner(t: npt.NDArray[np.floating]) -> npt.NDArray[np.floating]:
    """Evaluate Fresnel integrals via Chebyshev-optimized Horner scheme.
...
def _fresnel(
    ...
    """Fresnel integral using precomputed Chebyshev coefficients.

    The n_iter parameter is accepted for compatibility but ignored.
    """
    t = np.linspace(0, s / float(np.sqrt(2) * R0), num_pts)
    return cast("npt.NDArray[np.floating]", np.sqrt(2) * R0 * _fresnel_horner(t))


def _fresnel_angular(
    R0: float, s: float, num_pts: int, n_iter: int = 8
) -> npt.NDArray[np.floating]:
    """Fresnel integral with uniform angular sampling and Chebyshev coefficients.
