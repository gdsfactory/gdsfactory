...
def extrude(
    ...
    for section in x.sections:
        ...
        if section.insets and section.insets != (0, 0):
            ...
            p_segment_lengths_forward_cumsum = np.cumsum(p_segment_lengths)  # To get start inset idx & path length
            p_segment_lengths_reverse_cumsum = np.cumsum(p_segment_lengths[::-1])  # To get stop inset idx & path length

            if all(section.insets[:] > p_segment_lengths_forward_cumsum[-1]):
                warnings.warn(f"Cannot apply delay to Section '{section.name}', delay results in points outside "
                              f"of original path.",
                              stacklevel=3,
                              )
                continue
            ...
            '''
            start_diff_idx = np.argwhere(p_segment_lengths_forward_cumsum >= section.insets[0])[0, 0]
            ...
            e.g.)   For a straight path (chosen because I don't know how to draw a representation of a curved path 
                    ...
                    ---  ---  ---  ---  ---  ---
                         <--                 -->
                          ^(v_start)  (v_stop)^
            '''
            v_start = -p_xy_segment_lengths[start_diff_idx, :]  # Reversing vector direction so points inside-out
            v_stop = p_xy_segment_lengths[stop_diff_idx, :]  # Vector already points inside-out
            ...
            '''
            start_inset_remainder = p_segment_lengths_forward_cumsum[start_diff_idx] - section.insets[0]
            stop_inset_remainder = p_segment_lengths_reverse_cumsum[reversed_stop_diff_idx] - section.insets[1]
