...
def cross_section(
    ...
    section_list = sections if sections is not None else ()
    s = [
        Section(
            width=width,
            offset=offset,
    ...
    if section_list:
        # Only extend if there are extra sections
        s.extend(section_list)

    # Pre-compute these to avoid redundant checks and allocations
    add_cladding = cladding_layers is not None
    if add_cladding:
        n_clads = len(cladding_layers)
        cladding_offsets_not_none = (
            cladding_offsets if cladding_offsets is not None else (0,) * n_clads
        )
        cladding_simplify_not_none = (
            cladding_simplify if cladding_simplify is not None else (None,) * n_clads
        )
        cladding_centers_not_none = (
            cladding_centers if cladding_centers is not None else (0,) * n_clads
        )

        # Only length check, directly
        n_offsets = len(cladding_offsets_not_none)
        n_simplify = len(cladding_simplify_not_none)
        n_centers = len(cladding_centers_not_none)
        if not (n_clads == n_offsets == n_simplify == n_centers):
            raise ValueError(
                f"{n_clads=}, "
                f"{n_offsets=}, "
                f"{n_simplify=}, "
                f"{n_centers=} must have same length"
            )

        # Add all cladding sections efficiently
        s += [
            Section(
            ...
            for i, (layer_, offset, simplify, center) in enumerate(
                zip(
                    cladding_layers,
                    cladding_offsets_not_none,
                    cladding_simplify_not_none,
