...
def bezier(
    ...
    c.info["start_angle"] = pp.snap.snap_to_grid(angles[0])
    c.info["end_angle"] = pp.snap.snap_to_grid(angles[-2])
    ...
    length = pp.snap.snap_to_grid(path_length(path_points))
    min_bend_radius = pp.snap.snap_to_grid(1 / max(np.abs(curv)))
